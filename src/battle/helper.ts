 ...
 */
export const calculateTBR = ({ myTbr, opponentsTbr, result }: { myTbr: number, opponentsTbr: number, result: TBRResult }) => {
 ...
 * @param tbr 
 */
export const getLeague = (tbr: number) => {

    if (tbr < 1597) return "Tiny Explorer League";
    if (tbr >= 1597 && tbr < 2126) return "Tiny Fighter League";
    if (tbr >= 2126 && tbr < 2830) return "Tiny Scholar League";
    if (tbr >= 2829 && tbr < 3766) return "Tiny Master League";
    if (tbr >= 3766 && tbr < 4557) return "Tiny Radiant League";
